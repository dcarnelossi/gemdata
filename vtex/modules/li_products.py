...
def make_request_lojaintegrada(idproduto):
    try:
        url = f'https://api.awsli.com.br/v1/produto/{idproduto}'
...
def get_ids_products():
    query = f"SELECT id FROM lojaintegrada_list_products where data_insercao >= '{start_date_info}' ORDER BY id"

    logging.info(query)
    result = WriteJsonToPostgres(data_conection_info, query, "lojaintegrada_products")
    rows, _ = result.query()
    return [row[0] for row in rows]


def get_api_data(idproduto):
    try:
        return make_request_lojaintegrada(idproduto)
