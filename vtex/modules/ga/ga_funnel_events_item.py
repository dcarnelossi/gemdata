...
def run_report_ga(start_date, end_date):
    try:
        ...
        for row in response.rows:
            ...
            # === Dimensões
            for idx, dim in enumerate(["date", "itemId", "itemName", "eventName"]):
                if dim == "date":
            ...
            # === Métricas
            for idx, metric in enumerate(["itemsViewed", "itemsAddedToCart","itemsPurchased", "itemRevenue"]):
        ...
        # Converte métricas para numérico e preenche valores ausentes com 0
        metric_cols = ["itemsViewed", "itemsAddedToCart", "itemsPurchased", "itemRevenue"]
        for col in metric_cols:
            df[col] = pd.to_numeric(df[col], errors="coerce").fillna(0)

        # Agrupa por data + itemId + itemName
        grouped = df.groupby(["creationdate", "itemId", "itemName"], as_index=False)[metric_cols].sum()
...
def  execute_process_ga (start_date, end_date):
    ...
    if data_ga4:  # só tenta processar se tiver dados
        ...
        table = "ga_funnel_events_item"
        keytable = "creationdate, itemid,itemname"
