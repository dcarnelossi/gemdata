...
def run_report_ga(start_date, end_date):
    try:
        request = RunReportRequest(
            ...
            dimensions=[Dimension(name="date"), Dimension(name="eventName"),Dimension(name="city"),Dimension(name="region")],
        ...
        for row in response.rows:
            row_data = {
                "creationdate": row.dimension_values[0].value,
...
def  execute_process_ga (start_date, end_date):
    ...
    if data_ga4:  # só tenta processar se tiver dados
        ...
        table = "ga_funnel_events_region"
        keytable = "creationdate, event_name,city,region"
