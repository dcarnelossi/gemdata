...
def get_categories_tree(category_levels):
    try:
        ...
        if res.status == 200:
            ...
            #save_json_to_blob_storage("categories",f"categories", data.decode("utf-8"))
...
def get_category_id(category_id):
    try:
        conn = http.client.HTTPSConnection(VTEX_Domain)
        conn.request("GET", f"/api/catalog/pvt/category/{category_id}", headers=headers)
        res = conn.getresponse()
        data = res.read()

        if res.status == 200:
            print(data.decode("utf-8"))
            # save_json_to_blob_storage("categories",f"category_{category_id}", data.decode("utf-8"))
            # save_json_to_cosmosdb("categories", data.decode("utf-8"))
            
            writer = insdata.WriteJsonToPostgres(json.loads(data.decode("utf-8")), 'categories')
        ...
        print(f"get_category_id - {category_id} - An unexpected error occurred: {e}")
        return None
    finally:
        conn.close()
...
#extract_category_ids(json_data)
