...
def get_details_orders(column_orders,table_ref):
    query = f"SELECT {column_orders},order_number  FROM {table_ref} where data_insercao >= '{start_date_info}' ORDER BY id"


    logging.info(query)
    result = WriteJsonToPostgres(data_conection_info, query, {table_ref})
    rows, _ = result.query()
    return rows
...
def fetch_and_process(query_type,table_ref):
    try:
        json_details_orders = get_details_orders(query_type,table_ref)
        ...
        for row in json_details_orders:
            ...
            for pagamento in pagamentos:
                pagamento["order_number"] = order_number
...
def set_globals(api_info, data_conection, coorp_conection, type_api,start_date,table_ref):
    ...
    fetch_and_process(type_api,table_ref)



#set_globals("a","integrations-data-dev","appgemdata-homol","shippings","moovin_orders")
#set_globals("a","integrations-data-dev","appgemdata-homol","payments","moovin_orders")

# set_globals("a","integrations-data-dev","appgemdata-homol","items","moovin_order_shippings")