...
def make_request_moovin(offset):
    try:
        ...
        if isdaily_info:
            url = f'https://api.moovin.store/oms-order/order?updatedAt={start_date_info}&size={LIMIT}&offset={offset}'
        else:
            url = f'https://api.moovin.store/oms-order/order?createdAt={start_date_info}&size={LIMIT}&offset={offset}'
