    ...
    try:
        ...
        if res.status == 200:
            ...
            for category_id in extract_category_ids(data.decode("utf-8")):
                ...
                if error_ids:
                    logging.error(f"Error fetching data for the following product IDs: {error_ids}")
            return result
        else:
...
def get_product_variations(product_id):
    try:
        ...
        conn.request("GET", f"/api/catalog_system/pub/products/variations/{product_id}", headers=headers)
...
def check_and_create_table(category_data):
    ...
    if first_product_id is not None:
        ...
        writer = insdata.WriteJsonToPostgres(product_data, 'products')
...
def process_product_data(product_id, error_ids):
    try:
        ...
        if product_data is not None:
            ...
            writer = insdata.WriteJsonToPostgres(json.loads(product_data), 'products')
        ...
        if res.status == 200:
            ...
            if product_data is not None:
                ...
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    executor.map(lambda product_id: process_product_data(product_id, error_ids), extract_product_ids(data.decode("utf-8")))

                # Log error IDs to the file
                if error_ids:
                    logging.info(f"Error fetching data for the following product IDs: {error_ids}")
...
def normalise_products_by_id(product_id):
    try:
        ...
        if json_product["IsActive"]:
            ...
            if json_variations_str:
                try:
                    ...
                    logging.error(f"Error decoding JSON for variations of product {product_id}: {e}")
            else:
        ...
        logging.error(f"get_product_id - {product_id} - An unexpected error occurred: {e}")
        return None
