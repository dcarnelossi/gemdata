...
#    render_template_as_native_obj=True,
    ...
    def get_integration_ids(**kwargs):
        try:
            ...
            # Armazena os IDs no contexto
            kwargs['ti'].xcom_push(key='integration_ids', value=[integration[0] for integration in integration_ids])
    ...
    @task(provide_context=True)
    def trigger_import_dags(**kwargs):
        # Recupera os IDs do contexto
        integration_ids = kwargs['ti'].xcom_pull(key='integration_ids')
    ...
    get_integration_ids() >> trigger_import_dags()