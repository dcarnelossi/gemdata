...
def check_previous_dag_success(dag_id, **kwargs):
    try:
        ...
        return (
            f"trigger_next_{dag_id}"
            if prev_dag_state == "success"
            else f"skip_next_{dag_id}"
        )
...
def create_skip_dag_task(dag_id, dag):
    try:
        ...
        logging.error(
            f"Error creating EmptyOperator for skipping DAG {dag_id}: {str(e)}"
        )
        raise
...
def create_dag_tasks(dag_id, dag, schema):
    ...
    trigger_next_dag_task = create_trigger_dag_task(
        dag_id, f"{dag_id}_to_execute", dag, schema
    )
...
with dag:
    ...
    tasks = (
        ...
        + [
            task
            for dag_id in dags_to_execute
            for task in create_dag_tasks(dag_id, dag, "{{ params.schema }}")
        ]
        + [end_task]
    )
